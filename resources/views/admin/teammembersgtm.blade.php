<x-app-layout class="">   
    ...
    <div class="flex flex-col md:flex-row justify-around items-center gap-6 mt-6">
        <div>  
            <a href="/admin/team-members/gtm">
                <button id="gtm-label" class="button text-center border py-2 px-4 rounded-md shadow color-2">General Team Member</button>
            </a>
        </div>
        <div>
    ...
    <div class="relative overflow-x-scroll scroll shadow-md sm:rounded-lg ">
        <div class="flex">
            ...
            <div class="p-4">
                <a href="/admin/team-members/gtm/create">
                    <label for="table-search" class="sr-only">Search</label>
                    <div class="mt-1">
        ...
        <table class="w-full text-sm text-left text-gray-500 ">
            ...
            <tbody>
                @foreach ($teammembersgtm as $item)
                <tr class="bg-white  hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4">
                        {{ $item->team_member_gtm->designation}}    
                    </td>
                    <td class="px-6 py-4">
                        {{ $item->team_member_gtm->portfolio }}    
                    </td>
                    <td class="px-6 py-4">
                        {{ explode(" ", $item->created_at)[0] }}
                    </td>
                    <td class="px-6 py-4 text-right">
                        {{ Form::open(array('route' => array('gtm.edit', $item->id), 'method' => 'get', 'class'=>'inline')) }}
                            <button type="submit" >Edit</button>
                        {{ Form::close() }}
                        | 
                        {{ Form::open(array('route' => array('gtm.destroy', $item->id), 'method' => 'delete', 'class'=>'inline')) }}
